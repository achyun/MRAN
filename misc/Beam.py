...
class Beam():
    ...
    def __init__(self, size, max_len, device=False, all_init=False, specific_nums_of_sents=0):
        ...
        self.next_ys = [torch.full((size,), cfg.train.constants['PAD'], dtype=torch.long, device=device)]
        if all_init:
            # LSTM
            for i in range(size):
                self.next_ys[0][i] = cfg.train.constants['BOS']
        else:
            # ARFormer
            self.next_ys[0][0] = cfg.train.constants['BOS']
        self.finished = []
    ...
    def advance(self, word_prob):
        ...
        if len(self.prev_ks) > 0:
            ...
            for i in range(self.next_ys[-1].size(0)):
                if self.next_ys[-1][i] == cfg.train.constants['EOS']:
                    beam_lk[i] = -1e20
        else:
            beam_lk = word_prob[0]
    ...
    def get_tentative_hypothesis(self, so_far=True):
        ...
        if len(self.next_ys) == 1:
            ...
            if so_far:
                hyps = [[cfg.train.constants['BOS']] + h for h in hyps]
