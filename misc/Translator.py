...
import os, json
from utils import auto_enlarge


class Translator(object):
    ''' Load with trained model and handle the beam search '''

    def __init__(self, model, cfg, device=torch.device('cuda'), teacher_model=None, dict_mapping={}):
        self.model = model
        self.model.eval()
        self.cfg = cfg
        self.device = device
    ...
    def collect_hypothesis_and_scores(self, inst_dec_beams, n_best):
        ...
        for inst_idx in range(len(inst_dec_beams)):
            scores, tk = inst_dec_beams[inst_idx].sort_finished(1.0)
    ...
    def translate_batch(self,joint_rep, category):
        ...
        def beam_decode_step(
            ...
            def predict_word(dec_seq, inputs_for_decoder, n_active_inst, n_bm):
                dec_output, *_ = self.model.decoder(joint_rep=inputs_for_decoder['enc_output'],
                                                    tag_tokens=dec_seq,
                                                    category=inputs_for_decoder['category'])
                if isinstance(dec_output, list):
                    dec_output = dec_output[-1]
                dec_output = dec_output[:, -1, :]  # Pick the last step: (bh * bm) * d_h

                word_prob = self.model.tag_proj(dec_output)
        ...
        with torch.no_grad():
            inputs_for_decoder = {'enc_output': joint_rep, 'category': category}
            # -- Repeat data for beam search
            n_bm = self.cfg.test.beam_size
            ...
            # -- Prepare beams
            inst_dec_beams = [
                Beam(n_bm, self.cfg.train.token_max_len, device=self.device, specific_nums_of_sents=1) for
                _ in range(n_inst)]
            ...
            # -- Decode
            for len_dec_seq in range(1, self.cfg.train.token_max_len):
